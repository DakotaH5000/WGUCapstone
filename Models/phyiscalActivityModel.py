...
def run_physicalActivity_model(*args, **kwargs):
...
# Now use pandas to read the CSV
    ...
    #If user input, use the user input to generate data frame to predict with
    if args:
    ...
    fig = px.bar(importance_df, x='Attribute', y='Importance', title='Attribute Weight for Physical Activity', labels={'Importance': 'Importance Score', 'Attribute': 'Attribute'})
    fig.show()

    predictions_df = pd.DataFrame({'Actual': y_test, 'Predicted': y_pred})
    #Return a scatter plot of data
