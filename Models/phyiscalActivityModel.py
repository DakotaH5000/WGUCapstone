...
def run_physicalActivity_model(*args, **kwargs):
    ...
    X = df.drop(df.columns[9], axis=1)  # All rows, all columns except the last
    y = df.iloc[:, 9]   # All rows, only the last column
    ...
    print(f'Predicted Mental Health Status: {predicted_status[0]}')