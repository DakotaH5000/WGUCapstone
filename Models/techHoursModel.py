...
# Now use pandas to read the CSV
    ...
    #If user input, use it to create a data frame, else default input
    if args:
        ...
        new_data = pd.DataFrame([[25, 1, 5, 2, 1, 6, 3, 7, 2, 1, 3, 0]], 
                            columns=['Age', 'Gender', 'Social_Media_Usage_Hours', 'Gaming_Hours', 
                                    'Screen_Time_Hours', 'Mental_Health_Status', 'Stress_Level', 'Sleep_Hours', 'Physical_Activity_Hours', 
                                    'Support_Systems_Access', 'Work_Environment_Impact', 'Online_Support_Usage'])
    # Use the model to predict techHours
    ...
    fig = px.bar(importance_df, x='Attribute', y='Importance', title='Attribute Weight for Tech Usage Hours', labels={'Importance': 'Importance Score', 'Attribute': 'Attribute'})
    fig.show()

    predictions_df = pd.DataFrame({'Actual': y_test, 'Predicted': y_pred})
    #Return a scatter plot to users.
